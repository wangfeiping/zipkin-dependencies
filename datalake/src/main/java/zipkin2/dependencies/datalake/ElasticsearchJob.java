    ...
    try {
      JavaRDD<Map<String, Object>> links =
          JavaEsSpark.esJsonRDD(sc, spanResource)
               ...
               * 
               * JSON_TRACE_ID:
               * 根据 Tuple2<String, String> pair._2的json中traceId对原始数据分组;
               * Tuple2 为二元组，每个二元组表示一个span的依赖数据
               * pair._1为traceId 初始状态为空？，pair._2为当前span json数据;
               */
              .groupBy(JSON_TRACE_ID)
              /**
               * Pass each value in the key-value pair RDD through a flatMap function without changing the
               * keys; this also retains the original RDD's partitioning.
               * 将根据traceId分组的数据映射（转换？）为 Iterable<DependencyLink> 数据格式,
               * 保存原有健值(traceId)和分区(partitioning)：
               * 建立跟踪树 SpanNode traceTree (一条完整链路)；
               * 广度优先遍历跟踪树 (为了保证不完全相同的链路，相同的部分结构/位置相同？)；
               * 对单次链路调用进行计数；
               */
              .flatMapValues(new TraceIdAndJsonToDependencyLinks(logInitializer, decoder))
              /**
               * Return an RDD with the values of each tuple.
               * 将DependencyLink 封装为RDD，
               */
              .values()
              /**
               * Return a new RDD by applying a function to all elements of this RDD.
               * 推断：将数据重新分布（修改RDD分区）？以便于进一步分布式计算？
               */
               ...
               * 将key相同的DependencyLink两两相加求和，进行归约计算
               */
              ...
              /**
               * Return a new RDD by applying a function to all elements of this RDD.
               * 将RDD DependencyLink 数据转换为 LinkedHashMap，以便于转为统一json格式存储？
               */
